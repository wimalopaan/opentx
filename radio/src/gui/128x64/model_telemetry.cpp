...
static uint8_t SENSOR_ROW(uint8_t value)
{
...
#define IS_RANGE_DEFINED(k)            (g_model.frsky.channels[k].ratio > 0)
  ...
  uint8_t SENSOR_ROWS(uint8_t x) { return SENSOR_ROW((isTelemetryFieldAvailable(x) ? (uint8_t)0 : HIDDEN_ROW)); }
...
#elif defined(PCBI6X) // telemetry input disabled
...
void menuModelTelemetry(event_t event)
{
  ...
  MENU(STR_MENUTELEMETRY, menuTabModel, MENU_MODEL_TELEMETRY, HEADER_LINE+ITEM_TELEMETRY_MAX, { HEADER_LINE_COLUMNS TELEMETRY_TYPE_ROWS CHANNELS_ROWS SENSORS_ROWS RSSI_ROWS USRDATA_ROWS VARIO_ROWS TELEMETRY_SCREENS_ROWS });
