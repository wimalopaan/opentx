...
void per10ms()
{
  ...
  if (IS_ROTARY_ENCODER_NAVIGATION_ENABLE())
  {
    ...
    if (scrollRE)
    {
      ...
      if ((g_tmr10ms - lastEvent >= 10) || (cw == new_cw))
      { // 100ms
  ...
  if (mixWarning & 1)
    if (((g_tmr10ms & 0xFF) == 0))
      AUDIO_MIX_WARNING(1);
  if (mixWarning & 2)
    if (((g_tmr10ms & 0xFF) == 64) || ((g_tmr10ms & 0xFF) == 72))
      AUDIO_MIX_WARNING(2);
  if (mixWarning & 4)
    if (((g_tmr10ms & 0xFF) == 128) || ((g_tmr10ms & 0xFF) == 136) || ((g_tmr10ms & 0xFF) == 144))
      AUDIO_MIX_WARNING(3);
#endif
  ...
  if (modelId != 0)
  {
    for (uint8_t i = 0; i < MAX_MODELS; i++)
    {
      if (i != index)
      {
        ...
        {
          if ((WARNING_LINE_LEN - 4 - (name - reusableBuffer.modelsetup.msg)) > (signed)(modelHeaders[i].name[0] ? zlen(modelHeaders[i].name, LEN_MODEL_NAME) : sizeof(TR_MODEL) + 2))
...
void checkTrims()
{
  ...
  if (event && !IS_KEY_BREAK(event))
  {
    ...
    if (thro)
      v = 4; // if throttle trim and trim trottle then step=4
#if defined(GVARS)
    if (TRIM_REUSED(idx))
      v = 1;
#endif
    ...
    if (!thro && before != 0 && ((!(after < 0) == (before < 0)) || after == 0))
    { //forcing a stop at centerered trim when changing sides
      after = 0;
      beepTrim = true;
      AUDIO_TRIM_MIDDLE();
      pauseEvents(event);
